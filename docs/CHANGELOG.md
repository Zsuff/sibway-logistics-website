...
### SEO-005 DONE: BreadcrumbList schema + перша публікація import-poland/germany/czechia на проді

- **SEO-005** → DONE: додано `BreadcrumbList` JSON-LD на 9 файлів
  (uk/en/pl × import-poland/germany/czechia) у `sibway-website-`
  (PR #17, squash-merge `1b61bc4`), задеплоєно на `sibway.com.ua`
  через FileZilla.
- **Важливе відкриття**: ці 3 route-сторінки насправді ще не були на
  продакшені (404 до заливки) — попередні записи в `TASKS.md` і
  проєктному брифі про «вже опубліковані route pages» були
  помилковими. Разом зі схемою це стало фактично **першою
  публікацією** трьох лендінгів. Підтверджено власником окремо перед
  заливкою.
- Разом задеплоєно відсутні зображення `img/routes/route-{poland,
  germany,czechia}-ukraine.svg` (без них — биті hero-картинки).
- Пост-перевірка: усі 9 HTML і 3 SVG → 200 на проді; JSON-LD валідний.
- **Відкрито**: `services.html` (uk/en/pl) ще не посилається на ці
  три сторінки з навігації послуг — окреме рішення, не зроблено без
  дозволу.

## 2026-09-25 (2)

### RES-002 DONE: ADR/температура/негабарит підтверджено власником

- **RES-002** → DONE: власник підтвердив, що Sibway реально й стабільно
  організовує через партнерів ADR-вантажі, температурний режим
  (рефрижератори) та негабаритні перевезення. Раніше це було
  непідтвердженим твердженням із зовнішнього SEO/AEO/GEO-плану —
  тепер це підтверджений факт, зафіксований у пам'яті проєкту
  (`claude/sibway-business-profile`).
- Розблоковує `SEO-006` (аудит route pages) і `SEO-007` (пріоритизація
  supporting-контенту) — ці сценарії можна використовувати на сайті.
  Самі текстові зміни на сайті — окрема задача з окремим дозволом.
